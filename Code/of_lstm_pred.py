...
class App:
    ...
    def plot_accidentPos(self,pos,vis,text):
        ...
        if not self.detectedIndicator :
            
            img = cv2.rectangle(vis , start_point , end_point , color,thickness)
            img = cv2.putText(img, text , (pos[0], int(round(pos[1]-w*0.08))), cv2.FONT_HERSHEY_SIMPLEX, 0.9, (36,255,12), 2)
            self.accidentPos.append(img)
            self.detectedIndicator = True 
    ...
    def CheckAccident(self,frame_gray,vis):
        ...
        threshold_speed = 20 #The treshold is suited for a particular environmenet ie video 
        threshold_trajectory = 2
        ...
        for idx, tr in enumerate(self.tracks):
            if(len(self.savedPredictions[idx]) == 10 ) : #checking if there is enough information to compare
                ...
                if traj > threshold_trajectory :
                    self.plot_accidentPos(tr[-1],vis, "Traj Anomaly")
                
                speedDiff = np.linalg.norm(predictedTraj[-1]- predictedTraj[0]) - np.linalg.norm(realTraj[-1]-realTraj[0]) 
                
                if speedDiff > threshold_speed :
                    self.plot_accidentPos(tr[-1],vis, "Speed Anomaly")
